// ===============================
// FAST BIP32 SCANNER v15 - WSZYSTKIE MONETY (KOMPRESOWANE + NIEKOMPRESOWANE)
// ===============================
...
// ===============================
// TEST DLA PRIVATE KEY = 1 (WSZYSTKIE MONETY - KOMPRESOWANE + NIEKOMPRESOWANE)
// ===============================
void test_private_key_one() {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║     🔑 TEST DLA PRIVATE KEY = 1 (KOMPRESOWANE + NIEKOMPRESOWANE) ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n";
    std::cout << "\n";
    ...
    size_t pub_len = 33;
    secp256k1_ec_pubkey_serialize(ctx, pub_comp, &pub_len, &pub, SECP256K1_EC_COMPRESSED);
    
    unsigned char h160_comp[20];
    pubkey_hash160(pub_comp, pub_len, h160_comp);
    
    // ==========================================
    // NIEKOMPRESOWANY PUBKEY (65 bajtów)
    // ==========================================
    unsigned char pub_uncomp[65];
    pub_len = 65;
    secp256k1_ec_pubkey_serialize(ctx, pub_uncomp, &pub_len, &pub, SECP256K1_EC_UNCOMPRESSED);
    
    unsigned char h160_uncomp[20];
    pubkey_hash160(pub_uncomp, pub_len, h160_uncomp);
    
    // ==========================================
    // 1. BITCOIN P2PKH (1...) - KOMPRESOWANY
    // ==========================================
    std::cout << "💰 Bitcoin P2PKH (1...) [COMPRESSED]: ";
    std::string addr = ripemd_to_base58(h160_comp, 0x00);
    std::cout << addr << "\n";
    std::cout << "  Oczekiwany: 1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH\n";
    std::cout << "  " << (addr == "1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 2. BITCOIN P2PKH (1...) - NIEKOMPRESOWANY
    // ==========================================
    std::cout << "💰 Bitcoin P2PKH (1...) [UNCOMPRESSED]: ";
    std::string addr_uncomp = ripemd_to_base58(h160_uncomp, 0x00);
    std::cout << addr_uncomp << "\n";
    std::cout << "  Oczekiwany: 1EHNa6Q4Jz2uvNExL497mE43ikXhwF6kZm\n";
    std::cout << "  " << (addr_uncomp == "1EHNa6Q4Jz2uvNExL497mE43ikXhwF6kZm" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 3. BITCOIN P2SH (3...)
    // ==========================================
    unsigned char redeem_script_comp[22];
    redeem_script_comp[0] = 0x00;
    redeem_script_comp[1] = 0x14;
    memcpy(redeem_script_comp + 2, h160_comp, 20);
    unsigned char redeem_hash_comp[20];
    pubkey_hash160(redeem_script_comp, 22, redeem_hash_comp);
    
    std::cout << "💰 Bitcoin P2SH (3...): ";
    std::string addr_p2sh = ripemd_to_base58(redeem_hash_comp, 0x05);
    std::cout << addr_p2sh << "\n";
    std::cout << "  Oczekiwany: 3JvL6Ymt8MVWiCNHC7oWU6nLeHNJKLZGLN\n";
    std::cout << "  " << (addr_p2sh == "3JvL6Ymt8MVWiCNHC7oWU6nLeHNJKLZGLN" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 4. BITCOIN SEGWIT (bc1...)
    // ==========================================
    ...
    // ==========================================
    // 5. LITECOIN P2PKH (L...) - KOMPRESOWANY
    // ==========================================
    std::cout << "💰 Litecoin P2PKH (L...) [COMPRESSED]: ";
    std::string addr_ltc = ripemd_to_base58(h160_comp, 0x30);
    std::cout << addr_ltc << "\n";
    std::cout << "  Oczekiwany: LVuDpNCSSj6pQ7t9Pv6d6sUkLKoqDEVUnJ\n";
    std::cout << "  " << (addr_ltc == "LVuDpNCSSj6pQ7t9Pv6d6sUkLKoqDEVUnJ" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 6. LITECOIN P2PKH (L...) - NIEKOMPRESOWANY
    // ==========================================
    std::cout << "💰 Litecoin P2PKH (L...) [UNCOMPRESSED]: ";
    std::string addr_ltc_uncomp = ripemd_to_base58(h160_uncomp, 0x30);
    std::cout << addr_ltc_uncomp << "\n";
    std::cout << "  Oczekiwany: LYWKqJhtPeGyBAw7WC8R3F7ovxtzAiubdM\n";
    std::cout << "  " << (addr_ltc_uncomp == "LYWKqJhtPeGyBAw7WC8R3F7ovxtzAiubdM" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 7. LITECOIN P2SH (M...)
    // ==========================================
    std::cout << "💰 Litecoin P2SH (M...): ";
    std::string addr_ltc_p2sh = ripemd_to_base58(redeem_hash_comp, 0x32);
    ...
    // ==========================================
    // 9. DOGECOIN P2PKH (D...) - KOMPRESOWANY
    // ==========================================
    std::cout << "💰 Dogecoin P2PKH (D...) [COMPRESSED]: ";
    std::string addr_doge = ripemd_to_base58(h160_comp, 0x1E);
    std::cout << addr_doge << "\n";
    std::cout << "  Oczekiwany: DFpN6QqFfUm3gKNaxN6tNcab1FArL9cZLE\n";
    std::cout << "  " << (addr_doge == "DFpN6QqFfUm3gKNaxN6tNcab1FArL9cZLE" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 10. DOGECOIN P2PKH (D...) - NIEKOMPRESOWANY
    // ==========================================
    std::cout << "💰 Dogecoin P2PKH (D...) [UNCOMPRESSED]: ";
    std::string addr_doge_uncomp = ripemd_to_base58(h160_uncomp, 0x1E);
    std::cout << addr_doge_uncomp << "\n";
    std::cout << "  Oczekiwany: DJRU7MLhcPwCTNRZ4e8gJzDebtG1H5M7pc\n";
    std::cout << "  " << (addr_doge_uncomp == "DJRU7MLhcPwCTNRZ4e8gJzDebtG1H5M7pc" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 11. DASH P2PKH (X...) - KOMPRESOWANY
    // ==========================================
    std::cout << "💰 Dash P2PKH (X...) [COMPRESSED]: ";
    std::string addr_dash = ripemd_to_base58(h160_comp, 0x4C);
    std::cout << addr_dash << "\n";
    std::cout << "  Oczekiwany: XmN7PQYWKn5MJFna5fRYgP6mxT2F7xpekE\n";
    std::cout << "  " << (addr_dash == "XmN7PQYWKn5MJFna5fRYgP6mxT2F7xpekE" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 12. DASH P2PKH (X...) - NIEKOMPRESOWANY
    // ==========================================
    std::cout << "💰 Dash P2PKH (X...) [UNCOMPRESSED]: ";
    std::string addr_dash_uncomp = ripemd_to_base58(h160_uncomp, 0x4C);
    std::cout << addr_dash_uncomp << "\n";
    std::cout << "  Oczekiwany: XoyDQM3xGhFW5JqYBwTLckjqZ67Q3jZfAL\n";
    std::cout << "  " << (addr_dash_uncomp == "XoyDQM3xGhFW5JqYBwTLckjqZ67Q3jZfAL" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    ...
    zec_redeem[0] = 0x00;
    zec_redeem[1] = 0x14;
    memcpy(zec_redeem + 2, h160_comp, 20);
    ...
    std::cout << "  Oczekiwany: t3bnw6tC26gH7JqRB8YcdbutFtwZP4Xp5o5\n";
    std::cout << "  " << (addr_zec_p2sh == "t3bnw6tC26gH7JqRB8YcdbutFtwZP4Xp5o5" ? "✅ OK" : "❌ BŁĄD") << "\n\n";
    
    // ==========================================
    // 15. ETHEREUM - POPRAWIONE! (Tylko test, NIE SZUKA W PLIKU!)
    // ==========================================
...
// ===============================
void scan_seed_direct(secp256k1_context* ctx, const MMapFile& mm, 
                       const unsigned char priv[32], const std::string& priv_hex, 
                       std::ofstream& found) {
    // ==========================================
    // BEZ secp256k1_context_create/destroy!
    // ==========================================
    ...
    // ==========================================
    // SPRAWDŹ TYLKO 3 HASHE!
    // ==========================================
    bool found_comp = contains_address(mm, h160_comp);
    bool found_uncomp = contains_address(mm, h160_uncomp);
    bool found_p2sh = contains_address(mm, redeem_hash);
    bool found_eth = contains_address(mm, eth_hash);
    
    // ==========================================
    // KOMPRESOWANY - ZNALAZŁ?
    // ==========================================
    if (found_comp) {
        ...
        // Bitcoin P2PKH (1...)
        found << "BTC: " << ripemd_to_base58(h160_comp, 0x00) << "\n";
        // Litecoin P2PKH (L...)
        found << "LTC: " << ripemd_to_base58(h160_comp, 0x30) << "\n";
        // Dogecoin P2PKH (D...)
        found << "DOGE: " << ripemd_to_base58(h160_comp, 0x1E) << "\n";
        // Dash P2PKH (X...)
        found << "DASH: " << ripemd_to_base58(h160_comp, 0x4C) << "\n";
        // Zcash P2PKH (t1...)
        found << "ZEC: " << zcash_to_base58(h160_comp, 0x1C, 0xB8) << "\n";
        // SegWit (bc1...) - TEN SAM HASH!
        std::vector<uint8_t> five_bit;
        five_bit.push_back(0);
        auto converted = convert_bits(h160_comp, 20, 8, 5, true);
        five_bit.insert(five_bit.end(), converted.begin(), converted.end());
        found << "SEGWIT: " << bech32_encode("bc", five_bit) << "\n";
        found << "---\n";
        found.flush();
        
        total_found += 6;
        std::cout << "\n✅ ZNALEZIONO! (compressed) 6 adresów!" << std::endl;
    }
    
    // ==========================================
    // NIEKOMPRESOWANY - ZNALAZŁ?
    // ==========================================
    if (found_uncomp) {
        std::lock_guard<std::mutex> lock(log_mutex);
        found << "PRIV: " << priv_hex << "\nPATH: direct_uncompressed\n";
        found << "BTC: " << ripemd_to_base58(h160_uncomp, 0x00) << "\n";
        found << "LTC: " << ripemd_to_base58(h160_uncomp, 0x30) << "\n";
        found << "DOGE: " << ripemd_to_base58(h160_uncomp, 0x1E) << "\n";
        found << "DASH: " << ripemd_to_base58(h160_uncomp, 0x4C) << "\n";
        found << "ZEC: " << zcash_to_base58(h160_uncomp, 0x1C, 0xB8) << "\n";
        found << "---\n";
        found.flush();
        total_found += 5;
        std::cout << "\n✅ ZNALEZIONO! (uncompressed) 5 adresów!" << std::endl;
    }
    ...
    if (found_p2sh) {
        std::lock_guard<std::mutex> lock(log_mutex);
        found << "PRIV: " << priv_hex << "\nPATH: direct_p2sh\n";
        found << "BTC P2SH: " << ripemd_to_base58(redeem_hash, 0x05) << "\n";
        found << "LTC P2SH: " << ripemd_to_base58(redeem_hash, 0x32) << "\n";
        found << "ZEC P2SH: " << zcash_to_base58(redeem_hash, 0x1C, 0xBD) << "\n";
        found << "---\n";
        found.flush();
        total_found += 3;
        std::cout << "\n✅ ZNALEZIONO! (p2sh) 3 adresy!" << std::endl;
    }
    ...
    // ==========================================
    // KONTEKST NIE JEST NISZCZONY - NISZCZY GO KONSUMENT!
    // ==========================================
    // BEZ secp256k1_context_destroy(ctx);
}
...
void consumer(const MMapFile& mm, const std::string& output_file) {
    ...
    while (true) {
        ...
        // ==========================================
        // PRZEKAZUJEMY KONTEKST!
        // ==========================================
        scan_seed_direct(ctx, mm, priv, priv_hex, found);
        total_processed++;
    }
    
    // ==========================================
    // KONTEKST NISZCZONY RAZ!
    // ==========================================
    secp256k1_context_destroy(ctx);
}
